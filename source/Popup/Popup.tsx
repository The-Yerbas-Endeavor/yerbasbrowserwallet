...
const Popup: React.FC = () => {
  ...
  const doCreate = async (e: any) => {
    ...
    const privKey = yerbas.randomPrivateKey();
    const encrypted = yerbas.encrypt(privKey, createAccountPassword);
    (encrypted as any).label = createAccountLabel;
    (encrypted as any).address = yerbas.toAddress(privKey);
  ...
  React.useEffect(() => {
    ...
    } else if (!yerbas.isWif(importAccountWif)) {
      setImportAccountWifError("Invalid private key.");
    } else if (
      accounts.some(
        (x: any) =>
          x.address === yerbas.toAddress(yerbas.fromWif(importAccountWif))
      )
    ) {
  ...
  const doImport = async (e: any) => {
    ...
    const privKey = yerbas.fromWif(importAccountWif);
    const encrypted = yerbas.encrypt(privKey, importAccountPassword);
  ...
  const doExport = (e: any) => {
    e.preventDefault();
    e.stopPropagation();
    const privKey = yerbas.decrypt(activeAccount, exportPassword);
    const address = yerbas.toAddress(privKey);
    if (address !== activeAccount.address) {
      ...
      setExportWif(yerbas.toWif(privKey));
    }
  };
  ...
  React.useEffect(() => {
    ...
    } else if (!yerbas.isAddress(signAddress)) {
      setSignAddressError("Invalid Yerbas address.");
    } else {
  ...
  React.useEffect(() => {
    if (signAmount === "") {
      ...
      BigInt(yerbas.toSatoshi(signAmount.toString())) <
      yerbas.DUST_THRESHOLD
    ) {
      setSignAmountError(
        "Min amount = " +
          satoshiToLocaleString(yerbas.DUST_THRESHOLD.toString())
      );
    } else if (
      BigInt(yerbas.toSatoshi(signAmount.toString())) >
      yerbas.UTXO_MAX_AMOUNT
    ) {
      setSignAmountError(
        "Max amount = " +
          satoshiToLocaleString(yerbas.UTXO_MAX_AMOUNT.toString())
      );
    } else {
  ...
  const doSign = async (e: any) => {
    ...
    const privKey = yerbas.decrypt(activeAccount, signPassword);
    const address = yerbas.toAddress(privKey);
    if (address !== activeAccount.address) {
    ...
    const vouts = [
      {
        address: signAddress,
        amount: BigInt(yerbas.toSatoshi(signAmount)),
      },
    ];
    const data = signData.length === 0 ? null : Buffer.from(signData, "utf8");

    let signedTx = null;
    let fee = yerbas.FEE_RATE;
    while (true) {
      const test = yerbas.createSignedRawTransaction(
        vins,
        vouts,
        data,
        fee,
      ...
      const requiredFee =
        ...
        yerbas.FEE_RATE; // 100 bytes allowance for errors.
      if (fee >= requiredFee) {
        signedTx = test;
        break;
      }
  ...
  return (
    <div id="popup">
      ...
      {activeAccount !== null && (
        <Container className="content">
          ...
          <Row xs={2} md={2} lg={2} className="actions">
            ...
            <Col>
              <a
                href={`https://explorer.yerbas.org/address/${activeAccount.address}`}
                target="_blank"
                rel="noreferrer"
              >
      ...
      <div className="section-footer">
        <span>© The Yerbas Endeavor - 2023</span>
      </div>
    </div>
  );
};
