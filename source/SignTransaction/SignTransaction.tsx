...
const SignTransaction: React.FC = () => {
  ...
  React.useEffect(() => {
    (async () => {
      ...
      if (activeAccount !== null) {
        ...
        while (true) {
          const testAccountPrivKey = yerbas.randomPrivateKey();
          const testAccountAddress = yerbas.toAddress(testAccountPrivKey);
  ...
  const onApprove = (e: any) => {
    ...
    const privKey = yerbas.decrypt(account, signPassword);
    ...
    const signedTx = yerbas.createSignedRawTransaction(
  ...
  const onEnd = (err: any, res: any) => {
    const bc_bg_popup = new BroadcastChannel("yerbas_bg_popup_" + id);
